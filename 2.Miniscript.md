...
```
El comando getdescriptorinfo me da el descriptor publico con el checksum, pero necesitamos la combinación que incluye la clave privada del wallet de Alice, por lo que no tomo el dato directamente de getdescriptorinfo, tomo solo el checksum y lo combino con el descriptor que incluye la clave privada.
```
extdescsumA=$descextA#$extsumA
```
En los casos anteriores cuando usamos createwallet para importar los descriptors añadiamos el argumento disable_private_keys. Pero en este caso el wallet si tiene private keys, luego usaremos el argumento blank=true:
```
...
```
Ya tenemos un wallet descriptorA que puede firmar la parte de la transacción de Alice.
